...
(
  cd test-repo
  [ `sh ../git_list_branches.sh | grep a_branch | cut -c1-3` == "[0]" ] || flunk "Branch was not prepended with id"
  [ `sh ../git_list_branches.sh | grep b_branch | cut -c1-3` == "[1]" ] || flunk "Branch was not prepended with id"
)
